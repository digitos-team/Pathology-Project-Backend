...
export const listExpensesController = asyncHandler(async (req, res) => {
    ...
    const result = await listExpensesService(adminId, query);

    res.json(
        new ApiResponse(200, result, "Expenses fetched successfully")
    );
});
...
export const getExpenseByIdController = asyncHandler(async (req, res) => {
    ...
    res.json(new ApiResponse(200, expense, "Expense details fetched successfully"));
});
...
export const downloadExpenseReportController = asyncHandler(async (req, res) => {
    ...
    const reportData = await getExpenseReportService(adminId, type, year, month);
    ...
    res.setHeader("Content-Disposition", `attachment; filename=Expense_Report_${year}_${type}.pdf`);
