...
export const createPatient = asyncHandler(async (req, res) => {
  ...
  const patient = await patientService.createPatient(req.body, req.user.userId, req.user.labId);
...
export const getPatientById = asyncHandler(async (req, res) => {
  const patient = await patientService.getPatientById(req.params.id, req.user.labId);
...
export const updatePatient = asyncHandler(async (req, res) => {
  const patient = await patientService.updatePatient(req.params.id, req.user.labId, req.body);
  if (!patient) throw new ApiError(404, "Patient not found");
...
export const searchPatient = asyncHandler(async (req, res) => {
  const patients = await patientService.searchPatient(req.user.labId, req.query);
